...
int main()
{
    ...
    set <char> FIRST[no_nonTerminals+no_terminals+1];   // FIRST set for all terminals and non terminals and for $   
    set <char> FOLLOW[no_nonTerminals];               // fOLLOW set only for non terminals
    set <char> FIRSTSET;
    
    char* parsingTable[no_nonTerminals][no_terminals+1];       // parsing table initiated with string NULL as initial value +1 for $
    ...
    int flag=0;
    char prev_char;
    cout<<endl<<"Parsing the input string:   "<<input<<endl;
    cout<<"Stack\tInput\tProduction"<<endl<<endl;
    ...
    cout<<" \t"<<ip<<"  \t"<<prev<<" -> "<<prod<<endl;
    if(stack_value=='$')
       cout<<endl<<"Input successfully accepted"<<endl;         

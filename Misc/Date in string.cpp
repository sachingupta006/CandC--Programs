...
int main()
{
    char date[16],copy[12];
    int i=0;
    a: cout<<"Enter the date in your format"<<endl;
    cin>>date;
    int len=strlen(date);
    for(int j=0;j<len;j++)
    {
    ...
    for(int i=4;i<l;i++)
    {
            year+=(copy[i]-48)*(int)pow(10,l-i-1);
    }
    if(day<1)
    {
    ...
    if(month==2||month==4||month==6||month==9||month==11)
    {
         if(day==31)
         {
                    cout<<"This month cannot have 31 days Date entered is invalid"<<endl;
